    ...
    <v-card class="d-flex" height="100%" style="overflow: hidden;" >
      <v-row>
        <v-col cols="6" class="py-0" >
          <v-img height="490" width="500" position="center center" :src="dataImage.images" :lazy-src="dataImage.image"  aspect-ratio="1" >
        ...
        <v-col cols="6">
          <v-card elevation="0">
            <v-card-text>
              ...
              <span class="px-auto text-right text--disabled">{{dataImage.timestamp}}</span>
            </v-card-text>
            <v-card-actions>
              <div class="d-flex">
                <v-btn icon @click="updateLike(dataImage._id)" :color="dataImage.isLike ? 'red' : 'blue'">
                  <v-icon>fas fa-heart</v-icon>
                </v-btn>
                <v-btn text class="btn-person" @click="changeView('likes')">
                  <span v-if="dataImage.reactions" class="px-auto text-right text--disabled">{{dataImage.reactions.length}} Like</span>
                </v-btn>
            ...
            <likesPublication  v-show="changeViewActive == 'likes'" :reactions="dataImage.reactions" @changeShowModal="show = $event" />
            <comments v-show="changeViewActive == 'comments'"  :publicationId="dataImage._id" @changeShowModal="show = $event" />
        </v-col>
      </v-row>
    </v-card>
  </v-dialog>
